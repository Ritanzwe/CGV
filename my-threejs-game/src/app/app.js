/*
 * Project :WebGL Car Configurator turned Blacklist Game
 * File: app.js
 * Description : Entry main file for Blacklist game app
 * Date : 10/21/2024
 * License : MIT
 * Author : RendercodeNinja (modified)
...
class App extends BaseEngine {
    ...
    onLoadCompleted() {
        ...
        $('#preloader .icon').remove();
        $('#preloader .title').text('Blacklist Racer');
        $('#preloader .desc').html('Unleash your inner rebel in a race for freedom on the treacherous streets of the Blacklist');
    ...
    skipIntro() {
        ...
        AnimUtils.fadeElementIn($('#screen-fader')[0], 900, { display: 'flex' }, (fader) => {
            ...
            $('#level-selection').css('display', 'block');
    ...
    setupLevelSelection() {
        ...
        levelButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                // Store selected level in local storage
                localStorage.setItem('selectedLevel', selectedLevel);
                // Redirect to the car selection page or the level
